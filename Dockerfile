# Базовый образ ЗАПИНЕН по digest намеренно: без пина `docker build` периодически
# перерезолвливает python:3.11-slim в новый digest, что инвалидирует кэш всех
# слоёв ниже — включая ~40-минутную компиляцию pjsua2. Обновлять осознанно.
FROM python:3.11-slim@sha256:1c06f14f1f45c37c7ba0563077e651f288b728eb4a227db32da92b52794ddb3e
