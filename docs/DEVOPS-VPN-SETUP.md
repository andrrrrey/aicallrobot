...
### 4.11. Автоподдержание туннеля — `scripts/vpn-watchdog.sh` (+ systemd timer)

`res-vpn-up.sh` — разовый ручной подъём. Чтобы туннель **не «падал молча»**
(типичный случай: strongSwan держит IPsec `ESTABLISHED`, а `ppp0` отвалился —
`ipsec statusall` «зелёный», но робот выдаёт `Network is unreachable` и все
номера уходят в «Перезвон»), в репозитории есть watchdog.

Он периодически проверяет **реальное** здоровье туннеля (`ppp0` **и** маршрут в
`192.168.0.0/24`, а не статус IPsec) и в окне доступа аккуратно восстанавливает
его, синхронизируя `SIP_LOCAL_IP`.

Установка (ставит systemd timer, прогон каждые 2 мин):
```bash
sudo /opt/ai-robot/scripts/install-vpn-watchdog.sh
```
Наблюдение:
```bash
...
- после `MAX_FAILURES` неудач подряд (по умолчанию 3) включает **предохранитель**
  и прекращает автопопытки до ручного сброса;
- вне окна ПН–ПТ 08:00–22:00 GMT+7 попыток не делает;
- при успешном подъёме сбрасывает счётчик и предохранитель.

Если сработал предохранитель (в логе `ПРЕДОХРАНИТЕЛЬ ВКЛЮЧЁН`) — разберитесь с
причиной (PPP-логин/пароль, `xl2tpd`, окно) и сбросьте:
```bash
sudo rm /var/lib/ai-robot-vpn/breaker /var/lib/ai-robot-vpn/consecutive_failures
```
Переопределить параметры — в `/etc/default/ai-robot-vpn`:
```
MAX_FAILURES=2
...
> Хук `ip-up.d` (4.10) и watchdog дополняют друг друга: хук мгновенно
> синхронизирует `SIP_LOCAL_IP` при подъёме `ppp0`, watchdog — поднимает сам
> туннель, когда он отвалился.
...
## 9. Эксплуатация
- **Держать туннель поднятым автоматически** — поставить watchdog (раздел 4.11):
  `sudo /opt/ai-robot/scripts/install-vpn-watchdog.sh`. Он сам восстанавливает
  `ppp0` при обрыве, с защитой от блокировки /24.
