...
_NO_LOOP_NODES: frozenset[str] = frozenset({
    ...
    # Просьбы подождать/повторить можно обрабатывать сколько угодно раз
    "hold_on", "repeat",
})
...
# ── Просьба подождать / остаться на линии ───────────────────────────────────────
# Корни слов ожидания. «Подождите», «минутку», «секундочку» сами по себе —
# это просьба подождать, а не перевод звонка. Робот отвечает «Да, я вас слушаю»
# и остаётся в текущей фазе (раньше «подождите» ошибочно запускало перевод/рестарт).
_WAIT_STEMS: tuple[str, ...] = ("подожд", "погод", "обожд", "минут", "секунд", "момент")

# Слова-филлеры, которые допустимы рядом с просьбой подождать
# (включая имя робота «Татьяна» — «Татьяна, подождите, Татьяна»).
_HOLD_FILLER: frozenset[str] = frozenset({
    "татьяна", "да", "и", "ну", "вот", "сейчас", "пожалуйста", "алло", "ало",
    "э", "эээ", "а", "ой", "давайте", "чуть", "чуток", "немного", "тут",
    "буквально", "одну", "одна", "пару", "капельку", "чуточку", "прошу",
})


def _is_hold_request(lower: str) -> bool:
    """Реплика — это просьба подождать без иного содержательного смысла.

    «Подождите», «минутку», «Татьяна подождите» → True.
    ...
    if not any(any(w.startswith(s) for s in _WAIT_STEMS) for w in words):
        return False
    residual = [
        w for w in words
        if not any(w.startswith(s) for s in _WAIT_STEMS) and w not in _HOLD_FILLER
    ]
    return not residual
...
class ScriptDialogueV2:
    ...
    async def _dispatch(self, state: V2SessionState, user_text: str) -> tuple[str, str]:
        ...
        # Просьба подождать («подождите», «минутку», «Татьяна подождите») —
        # короткое подтверждение без перевода звонка и без рестарта диалога.
