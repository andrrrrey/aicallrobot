...
Коды:
- transfer_to_lpr: секретарь говорит "соединяю", "переведу", "передаю трубку", "переключаю", "подождите", "минутку", "сейчас переведу"
- lpr_speaking: новый собеседник берёт трубку ("алло", "слушаю", "да", "я вас слушаю") — голос сменился
...
- call_back: "перезвоните", "позвоните позже", "через час", "после обеда перезвоните"
...
class ScriptDialogueV2:
    ...
    async def _handle_secretary(self, state: V2SessionState, user_text: str) -> tuple[str, str]:
        ...
        code = await self._classify_secretary(user_text, state.last_robot_text)

        if code in ("transfer_to_lpr", "lpr_speaking"):
    ...
    async def _handle_lpr_greeting(self, state: V2SessionState, user_text: str) -> tuple[str, str]:
        code = await self._classify_lpr_greeting(user_text, state.last_robot_text)
    ...
    async def _handle_lpr_main(self, state: V2SessionState, user_text: str) -> tuple[str, str]:
        ...
        code = await self._classify_lpr_main(user_text, state.last_robot_text)
    ...
    async def _classify_secretary(self, user_text: str, last_robot: str) -> str:
        prompt = _SECRETARY_PROMPT.format(
            user_text=user_text, last_robot=last_robot or "—"
        )
        return await self._classify(prompt, SECRETARY_INTENT_CODES)

    async def _classify_lpr_greeting(self, user_text: str, last_robot: str) -> str:
        prompt = _LPR_GREETING_PROMPT.format(
            user_text=user_text, last_robot=last_robot or "—"
        )
        return await self._classify(prompt, LPR_GREETING_INTENT_CODES)

    async def _classify_lpr_main(self, user_text: str, last_robot: str) -> str:
