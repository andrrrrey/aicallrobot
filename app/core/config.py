...
class Settings(BaseSettings):
    ...
    vad_end_pause_sec: float = 1.0         # пауза после обычной речи (сек)
    vad_end_pause_short_sec: float = 1.4   # пауза после короткой реплики (сек)
    ...
    # Отсчёт идёт от момента, когда робот РЕАЛЬНО замолчал (очередь
    # воспроизведения опустела), а не когда закончилась генерация TTS — иначе
    # робот переспрашивает, ещё не договорив предыдущую реплику.
    # Человеку нужно время, чтобы осмыслить вопрос и ответить: меньше 8-10 с
    # ставить нельзя, иначе робот «строчит» репликами и не даёт ответить.
    no_input_timeout_sec: float = 15.0
    # Пауза перед ВТОРЫМ и последующими шагами лестницы молчания (переспрос
    # вопроса → вежливое завершение). Больше первой: если собеседник не ответил
    # на короткое «Алло, вы меня слышите?», он либо отошёл, либо думает —
    # в обоих случаях торопить его бессмысленно.
    no_input_repeat_timeout_sec: float = 14.0
    # Робота перебили (barge-in), он замолчал — и собеседник тоже замолчал, так
    # и не сказав ничего разборчивого. Через столько секунд тишины робот мягко
    # просит повторить («Извините, повторите, пожалуйста, не расслышала вас»),
    # а не договаривает прежнюю реплику и не молчит.
    interrupt_reask_sec: float = 3.0
